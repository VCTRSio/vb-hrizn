...
/**
 * hrizn_content — matches docker/postgres/init/01-schema.sql column-for-column.
 * Sanctioned divergence: tenant_type native pg enum → string (the RLS predicate casts
 * (tenant_type)::text, so it is enum/string agnostic); the composite tenant FK on
 * ideacloud_id is NOT reproduced (plain uuid). Fail-closed tenant RLS is reproduced
 * verbatim from core's enforce_real_rls (a clean external install is never swept by
 * that core migration, so skipping RLS would leave the table with no isolation).
 */
return new class extends Migration
{
    private const T = 'hrizn_content';
    ...
    public function up(): void
    {
        if (! Schema::hasTable(self::T)) {
            Schema::create(self::T, function (Blueprint $t) {
                ...
                $t->uuid('tenant_id');
                $t->uuid('ideacloud_id');
                $t->text('hrizn_content_id')->nullable();
                $t->text('article_type');
                $t->text('content_intent')->nullable();
                $t->boolean('auto_compliance')->default(true);
                $t->boolean('auto_content_tools')->default(true);
                $t->text('status')->default('pending');
                $t->integer('progress_percent')->default(0);
                $t->text('progress_stage')->nullable();
                $t->text('compliance_status')->nullable();
                $t->integer('compliance_score')->nullable();
            ...
            DB::statement('CREATE INDEX idx_hrizn_content_active ON public.hrizn_content USING btree (tenant_type, tenant_id) WHERE deleted_at IS NULL');
        }
        $this->applyRls();
    }
